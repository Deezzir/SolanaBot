...
import * as meteora from './trade_meteora.js';
...
var MINT_METADATA: meteora.MeteoraMintMeta;
var IS_DONE = false;
var START_BUY = false;
...
const buy = async () => {
    ...
    while (!IS_DONE && !bought) {
        ...
        while (count > 0) {
            const buy_promise = meteora.Trader.buy_token(
                amount,
                WORKER_KEYPAIR,
                MINT_METADATA,
...
const sell = async () => {
    ...
    while (!sold) {
        ...
        while (count > 0 && balance !== undefined) {
            const sell_promise = meteora.Trader.sell_token(
                balance,
                WORKER_KEYPAIR,
                MINT_METADATA,
